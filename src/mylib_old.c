...
void mylib_print_param_list(instructionLine* List_of_param, int all,  FILE *file){
	...
	int j;
		...
		for(j=0; j<all; j++)	if(List_of_param[j].rank !=0) fprintf(file, "%s\n", List_of_param[j].InstLine);
	return;
}

/***
** Function to clear all copy in two lists
***/
void mylib_clear_copy(instructionLine *instrLineA, int allA,  instructionLine *instrLineB, int allB){
	int i,j;
	for(j=0; j<allA; j++){
		for(i =0; i<allB; i++){
			if(!strcmp(instrLineA[j].InstLine, instrLineB[i].InstLine)) 
				instrLineB[i].rank = 0;
		}
	}
	return;
...
operandLine* mylib_operandLine_find(operandLine* List_operandLine, char* index){
	...
	while(help_line->next!=NULL){
		if(!strcmp(help_line->next->tab[0], index)){
			return help_line ;
		}
		help_line = help_line->next;
	}
...
void mylib_fetch_register(operandLine* List_operandLine, instructionLine* List_of_fetch, int number, int rank){
	...
	for(j=1; j<=number; j++){
		for(i=1; i<=rank; i++){//il faudra savoir le nombre de ligne a remplir #RANK
			...
			help_operandLine->next->round = call_register(help_operandLine->next->tab, help_operandLine->next->sum - 1, &help_operandLine->next->round, output);
