...
/*
void mylib_else_Insn_i(InsnLine[] InsnLineA, int index, InsnLine out){

	int i;
	InsnLine help_InsnLine;// = malloc (sizeof(InsnLine));
	help_InsLine.strLine = (char*)malloc(STRING_SIZE*sizeof(char));
	help_InsnLine = InsnLineA;
	for(i=0; i<index; i++){
		help_InsnLine = help_InsnLine->next;
	}
...
void Insn_arrangements(arrgt_t* arr, int k, InsnLine* L, InsnLine* t) {
	...
	if(!t_second) {printf("error");exit(EXIT_FAILURE);}
	t_second->next = NULL;
	InsnLine t_third[n]; 
	...
	for(i=0;i<n-k;i++) {
		...
		strcpy(help_copy_InsnLine.strLine, help_copy_InsnLine_2->strLine);
